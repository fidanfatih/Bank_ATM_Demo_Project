...
"""
Bank ATM projesinde guncelleme yapilmistir. 
Bilgiler txt dosyasina tutulurken, ayni zamanda mongodb cloud serverda da tutulmustur.
"""
...
class Data:
    ...
    def dbWrite(self):
        ...
        coll.update_one({'_id':self.user_name}, {"$set":{'password': self.password,
                                                        'name': self.name,
    ...
    def dbDelete(self):
        ...
        db= client.bank_atm        
        coll = db["user_info"]
        coll.delete_one({'_id':self.user_name})                 
...
def readDB():
    ...
    with open('user_info.txt', 'r', encoding = "utf-8") as f:
        ...
        for line in f:
            ...
            user_name = attributes[0]
            password = attributes[1]
            name = attributes[2]
            surname = attributes[3]
            phone_num = attributes[4]
            email = attributes[5]
            balance = int(attributes[6])
            transactions = attributes[7].split(',')
            
            try:
                res = coll.insert_one({'_id': user_name,
                                        'password': password,
                                        'name': name,
                                        'surname': surname,
                                        'phone_num': phone_num,
